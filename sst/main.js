...
let practice_unfinished = true;
const num_trials_per_block = 40; //40;
const num_block = 5;
const num_trials_practice = 16;
const max_rt = 2000;
...
let stopNum_criterion_finishPractice = 1;  // 1 when debugging -1
let accuracy_criterion_finishPractice = 0.75;  // 0.75 when debugging -1

const fixation_size = 150;  // 1cm
const stimuli_go = ['right', 'left'];
...
for (let i = 0; i < practice_stimuli.length; i++) {
  if (practice_stimuli[i].stimuli_go == 'right') {
    practice_stimuli[i].correct_key = 'j';
    practice_stimuli[i].stimulus = right_arrow_path;
  } else {
    practice_stimuli[i].correct_key = 'f';
    practice_stimuli[i].stimulus = left_arrow_path;
  }
...
for (let i = 0; i < test_stimuli_all.length; i++) {
  if (test_stimuli_all[i].stimuli_go == 'right') {
    test_stimuli_all[i].correct_key = 'j';
    test_stimuli_all[i].stimulus = right_arrow_path;
  } else {
    test_stimuli_all[i].correct_key = 'f';
    test_stimuli_all[i].stimulus = left_arrow_path;
  }
...
var test = {
  ...
  response_start_time: fixation_duration,
...
const forceQuit_if = {
  ...
  conditional_function: function() {
    ...
    let rt_avg = last_block.select('rt').mean();
    let is_tooFast = rt_avg < 250;

    // criterion 2: too low accuracy for go trials
    ...
    let is_tooInaacurate = (num_trials_go_correct / num_trials_go) < .60;
...
const rest = {
  ...
  stimulus: function () {
    ...
    let num_trials_go = go_last_block.count();
    let num_omission_go = go_last_block.filter({ response: null }).count();
    //let omission_rate = Math.round((num_omission_go / num_trials_go) * 100);
    
    let rt_avg = Math.round(go_last_block.select('rt').mean());
...
const practice_feedback = {
  ...
  stimulus: function () {
    ...
    let num_go_trials = go_trials.count();
    let num_omissions = go_trials.filter({ response: null }).count();
    let omission_feedback = '';
    /*
    if (num_omissions == 0) {
      omission_feedback = "Go試行において，キーの押し損ねはありませんでした。";
    } else {
      let omission_rate = Math.round((num_omissions / num_go_trials) * 100);
      omission_feedback = 'Go試行において，' + omission_rate + '%のキーを押しそこねました。';
    };
    
    let rt_avg = Math.round(go_trials.select('rt').mean());*/
    let accuracy_go = Math.round(go_trials.select('correct').mean());
    console.log(accuracy_go);
...
const if_node_endPractice = {
  ...
  conditional_function: function() {
    if (practice_unfinished & counter % num_trials_practice == 0)
      return true;
    else {
      return false;
    }
